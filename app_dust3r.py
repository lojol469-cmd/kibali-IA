...
import sys
# Ajouter DUSt3R au path
sys.path.insert(0, '/home/belikan/dust3r')
...
# Open3D désactivé (incompatible Python 3.13)
try:
...
# Afficher le statut Open3D
if not OPEN3D_AVAILABLE:
    st.warning("⚠️ Open3D non disponible (Python 3.13 incompatible). Fonctionnalités de maillage 3D désactivées. Visualisation Plotly disponible.")
...
with col1:
    ...
    # Désactiver les options Open3D si non disponible
    if OPEN3D_AVAILABLE:
        ...
        generate_mesh = False
        mesh_method = "Poisson"
        st.info("💡 Maillage 3D désactivé (nécessite Open3D). Visualisation Plotly disponible.")
        ...
        if model is None:
            ...
            with st.spinner("Traitement en cours..."):
                try:
                    ...
                    if len(merged_pts3d) > 0 and OPEN3D_AVAILABLE:
                    ...
                    elif len(merged_pts3d) > 0 and not OPEN3D_AVAILABLE:
                        st.warning("⚠️ Visualisation Open3D non disponible. Utilisez la vue Plotly ci-dessous.")
                        # Export PLY sans Open3D
                        import struct
                        pcd_tmp_path = os.path.join(tempfile.gettempdir(), f"temp_pcd_{uuid.uuid4().hex}.ply")
                        with open(pcd_tmp_path, 'wb') as f:
                            # Header PLY
                            f.write(b"ply\n")
                            f.write(b"format binary_little_endian 1.0\n")
                            f.write(f"element vertex {len(merged_pts3d)}\n".encode())
                            ...
                            # Data
                            for i in range(len(merged_pts3d)):
                                x, y, z = merged_pts3d[i]
                                r, g, b = (merged_colors[i] * 255).astype(np.uint8)
